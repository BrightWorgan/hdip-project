...
test.describe("Login and add a new user, and then delete the user ", () => {
  test("Login and add a new user", async ({ page }) => {
    ...
    await test.step("Confirm add form structure / UI", async () => {
      // await page.getByRole('button', { name: 'Add Team ' }).click();
      const formHeader = await page.getByRole("heading", {
        name: "Add / Edit a new Team Member",
      });
    ...
    await test.step("Confirm new user has been added and all data is displaying", async () => {
      await page
        .getByRole("row", { name: "Playwright Endtoend" })
        .getByRole("checkbox")
        .check();
      await page
        .getByRole("row", { name: "Playwright Endtoend" })
      ...
      await page
        .getByRole("cell", { name: "Test", exact: true })
        .nth(3)
        .click();
    });
  });
  ...
  test("Login & add a new user", async ({ page }) => {
    ...
    await test.step("Confirm new user has been added to the table", async () => {
      await page
        ...
        .hover();
      await page
        .getByRole("row", { name: "Tester Test 1234567890" })
        .getByRole("checkbox")
        .check();
      await page
        .getByRole("row", { name: "Tester Test 1234567890" })
