...
import { checkToast, login } from "./util";
...
test.describe("Login and basic UI structure", () => {
  ...
  test("Sucessful login test", async ({ page }) => {
    ...
    await test.step("confim toast message", async () => {
      await checkToast(page, "Logged In");
    });
  });
  ...
  test("Base UI structure", async ({ page }) => {
    ...
    await test.step("Check Header text and logo", async () => {
      ...
      // header text
      const headerTitle = await page.getByRole("heading", {
        name: "Welcome to Sheet Scribe",
      });
      const headerSubtitle = await page.getByRole("heading", {
        name: "For all of your Traceable Archaeology and Heritage needs",
      });
    ...
    await test.step("Confrim Footer Structure", async () => {
      ...
      await expect(footerContainer).toHaveCSS(
        "background-color",
        "rgb(214, 210, 185)"
      );
  ...
  test("Project Page structure", async ({ page }) => {
    ...
    await test.step("Confirm Add Project Form layout", async () => {
      ...
      await page
        .getByText("Contract TypePrivatePublicField SchoolResearch")
        .hover();
  ...
  test("Profile Page structure", async ({ page }) => {
    ...
    await test.step("confirm User Profile Page structure", async () => {
      ...
      // finds table
      const tableHeader = await page.getByRole("heading", {
        name: "Finds Register:",
      });
      ...
      const contextNum = await page.getByRole("cell", {
        name: "Context Number",
      });
